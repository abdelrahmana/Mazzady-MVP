...
class NestedListAdaptor(//val itemClickedAction : (Contentx)->Unit,
...
) : RecyclerView.Adapter<NestedListAdaptor.ViewHolder>() {
    ...
    inner class ViewHolder(
        ...
        fun bindItems(
            ...
            itemData?.options?.map {
                //Log.v("looper_list",it?.parent.toString())
                if (it == subLists?.first) {
                    it?.data = subLists?.second
                }
                it?.data?.let {
                    itemBinding.nestedOptionsChild.setVisibilty(View.VISIBLE)
                    val nestedAdaptor = LeafAdaptorList(
                        /*subListInfo?.second?:ArrayList()*/
                        it ?: ArrayList(), //itemData.selectedOption?.data?:ArrayList(),
                        utilii,
                        callBackSelectionRoot, subLists,dispossibleObserver
                    )
                    utilii.setRecycleView(
                        itemBinding.nestedOptionsChild,
                        nestedAdaptor,
                        LinearLayoutManager.VERTICAL,
                        itemBinding.arrowSubCategory.context,
                        null,
                        false
                    )
                }
